      ...
      <main className="mx-auto max-w-6xl px-6 py-12">
        ...
        <div className="mt-16 grid gap-8 md:grid-cols-2">
          {plans.slice(0, 2).map((p) => (
            <div
              key={p.uuid}
              className="rounded-3xl border border-neutral-200 bg-white p-10 shadow-sm"
            >
              <div className="flex items-center justify-center rounded-2xl bg-neutral-100 py-12">
                <div className="h-14 w-14 rounded-xl border border-neutral-300 bg-white flex items-center justify-center text-neutral-500">
                  ▢
                </div>
              </div>
              ...
              <ul className="mt-5 space-y-3 text-neutral-700">
                ...
                {p.energy_rate_cents_per_kwh ? (
                  <li className="flex gap-3">
                    <span className="mt-2 h-1.5 w-1.5 rounded-full bg-neutral-900" />
                    <span>Energy rate: {p.energy_rate_cents_per_kwh}¢/kWh</span>
                  </li>
                ) : null}
                {p.export_rate_cents_per_kwh ? (
                  <li className="flex gap-3">
                    <span className="mt-2 h-1.5 w-1.5 rounded-full bg-neutral-900" />
                    <span>Export rate: {p.export_rate_cents_per_kwh}¢/kWh</span>
                  </li>
                ) : null}
                {p.plan_monthly_cost_dollars ? (
                  <li className="flex gap-3">
                    <span className="mt-2 h-1.5 w-1.5 rounded-full bg-neutral-900" />
                    <span>Base Fee: ${p.plan_monthly_cost_dollars}/mo</span>
                  </li>
                ) : null}
                {p.tdu_shortname ? (
                  <li className="flex gap-3">
                    <span className="mt-2 h-1.5 w-1.5 rounded-full bg-neutral-900" />
                    <span>{p.tdu_shortname} Delivery Charges: {p.delivery_rate_cents_per_kwh}¢/kWh</span>
                  </li>
                ) : null}
              </ul>
        ...
        {/* Later you can show all plans instead of just 2 */}
      </main>
    </div>
  );
}